...
class RequestLinks(db.Model): #this is automatically created when you append users to groups or groups to users
    id = db.Column(db.Integer, primary_key=True)
    requester_id = db.Column(db.Integer, db.ForeignKey('user.id'))
    group_id = db.Column(db.Integer, db.ForeignKey('group.id'))

    requester = db.relationship('User', backref='requestownership')
    group = db.relationship('Group', backref='requestownership')
...
class User(db.Model, UserMixin):
    ...
    requests = association_proxy('requestownership', #whenever a user joins a group, append it to their groups
                              'group',
                              creator=lambda c: RequestLinks(id, c.id))
